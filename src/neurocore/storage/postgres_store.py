...
class PostgresStore(BaseStore):
    ...
    def _save_record_connection(
        self, connection, record: MemoryRecord, signature: str
    ) -> None:
        connection.execute(
            """
    ...
    def _save_document_connection(
        ...
        connection.execute("DELETE FROM chunks WHERE document_id = %s", (document.id,))
        for chunk in chunks:
            connection.execute(
                """
    ...
    def delete_brain(self, brain_id: str) -> None:
        with self._connect() as connection:
            cursor = connection.execute(
                "DELETE FROM brains WHERE brain_id = %s",
                (brain_id,),
            )
