...
class MirroredStore(BaseStore):
    """Wrap local and cloud stores with cloud-primary mirrored writes."""
    ...
    def update_brain(self, brain_id: str, patch: dict[str, object]) -> BrainManifest:
        updated = self.cloud_store.update_brain(brain_id, patch)
        try:
    ...
    def archive_brain(self, brain_id: str, reason: str) -> BrainManifest:
        archived = self.cloud_store.archive_brain(brain_id, reason)
        try:
            if self.local_store.get_brain(brain_id, include_archived=True) is None:
    ...
    def delete_brain(self, brain_id: str) -> None:
        self.cloud_store.delete_brain(brain_id)
        try:
    ...
    def update_record(
        ...
        updated = self.cloud_store.update_record(item_id, patch, mode)
        try:
    ...
    def update_document(
        ...
        updated = self.cloud_store.update_document(item_id, patch, mode)
        try:
    ...
    def soft_delete(self, item_id: str, reason: str) -> None:
        self.cloud_store.soft_delete(item_id, reason)
        try:
    ...
    def hard_delete(self, item_id: str) -> None:
        self.cloud_store.hard_delete(item_id)
        try:
    ...
    def reindex(
        ...
        processed, failed, warnings = self.cloud_store.reindex(
            ids,
            scope=scope,
        ...
        try:
            local_processed, local_failed, local_warnings = self.local_store.reindex(
                ids,
                scope=scope,
                semantic_backend=semantic_backend,
                semantic_model_name=semantic_model_name,
            ...
            if local_failed:
                self._record_local_failure(
                    ...
                    RuntimeError(f"{local_failed} local items could not be reindexed"),
                )
    ...
    def record_audit(
        ...
        self.cloud_store.import_audit_event(event)
        try:
            self.local_store.import_audit_event(event)
        except Exception as exc:
            self._record_local_failure("record audit event", exc)
    ...
    def backfill_local_to_cloud(self) -> dict[str, int]:
        return _sync_store(self.local_store, self.cloud_store, clear_target=False)
